#!/usr/bin/env python3
"""Stage and optionally package the @sj-cohere/scodex npm module."""
...
CODEX_NPM_NAME = "@sj-cohere/scodex"
CODEX_PACKAGE_COMPONENT = "codex-package"

# `npm_name` is the local optional-dependency alias consumed by `bin/codex.js`.
# The underlying package published to npm is always `@sj-cohere/scodex`.
CODEX_PLATFORM_PACKAGES: dict[str, dict[str, str]] = {
    "codex-linux-x64": {
        "npm_name": "@sj-cohere/scodex-linux-x64",
        "npm_tag": "linux-x64",
    ...
    "codex-linux-arm64": {
        "npm_name": "@sj-cohere/scodex-linux-arm64",
        "npm_tag": "linux-arm64",
    ...
    "codex-darwin-x64": {
        "npm_name": "@sj-cohere/scodex-darwin-x64",
        "npm_tag": "darwin-x64",
    ...
    "codex-darwin-arm64": {
        "npm_name": "@sj-cohere/scodex-darwin-arm64",
    ...
    "codex-win32-x64": {
        "npm_name": "@sj-cohere/scodex-win32-x64",
        "npm_tag": "win32-x64",
    ...
    "codex-win32-arm64": {
        "npm_name": "@sj-cohere/scodex-win32-arm64",
        "npm_tag": "win32-arm64",
...
def copy_native_binaries(
    ...
    for target_dir in vendor_src.iterdir():
        ...
        if CODEX_PACKAGE_COMPONENT in components_set:
            ...
            executable_suffix = (
                ".exe" if target_dir.name.endswith("windows-msvc") else ""
            )
            packaged_executable = dest_target_dir / "bin" / f"codex{executable_suffix}"
            scodex_executable = dest_target_dir / "bin" / f"scodex{executable_suffix}"
            if packaged_executable.exists() and not scodex_executable.exists():
                packaged_executable.rename(scodex_executable)
        else:
