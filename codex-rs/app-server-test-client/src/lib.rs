...
struct Cli {
    ...
    /// spawning `scodex app-server` as a child process.
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `scodex app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/codex-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `scodex app-server --listen`.
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let codex_bin = codex_bin.unwrap_or_else(|| PathBuf::from("scodex"));
...
fn serve(codex_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    ...
    println!("started scodex app-server");
...
impl CodexClient {
    ...
    fn spawn_stdio_with_env(
        ...
        let stdin = codex_app_server
            .stdin
            .take()
            .context("scodex app-server stdin unavailable")?;
        let stdout = codex_app_server
            .stdout
            .take()
            .context("scodex app-server stdout unavailable")?;
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to scodex app-server")?;
                    return Ok(());
                }
                bail!("scodex app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from scodex app-server")?;
                if bytes == 0 {
                    bail!("scodex app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl Drop for CodexClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[scodex app-server exited: {status}]");
            return;
        }
