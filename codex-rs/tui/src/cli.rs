...
pub struct Cli {
    ...
    // Internal controls set by the top-level `scodex resume` subcommand.
    ...
    /// top-level `scodex resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `scodex fork` subcommand.
    // These are not exposed as user flags on the base `codex` command.
    #[clap(skip)]
    ...
    /// top-level `scodex fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
