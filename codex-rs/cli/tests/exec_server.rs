...
"#
    ...
    std::fs::copy(codex_utils_cargo_bin::cargo_bin("scodex")?, &executable)?;
...
"#
    ...
    let codex_bin = codex_utils_cargo_bin::cargo_bin("scodex")?;
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let codex_home = TempDir::new()?;
    let mut child = std::process::Command::new(codex_utils_cargo_bin::cargo_bin("scodex")?)
        .env("CODEX_HOME", codex_home.path())
