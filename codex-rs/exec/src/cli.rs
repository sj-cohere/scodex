...
#[command(
    version,
    override_usage = "scodex exec [OPTIONS] [PROMPT]\n       scodex exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
...
struct ResumeArgsRaw {
    ...
    // so "scodex resume --last <prompt>" treats the positional as a prompt, not a session id.
