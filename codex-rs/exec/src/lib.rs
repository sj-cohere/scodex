...
enum StdinPromptBehavior {
    ...
    /// `scodex exec` behavior for `scodex exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `scodex exec -`
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("codex_exec".to_string()) {
        tracing::warn!(
            ?err,
            "Failed to set scodex exec originator override {err:?}"
        );
    }
    ...
    if worktree {
        ...
        match command.as_ref() {
            Some(ExecCommand::Resume(_)) => {
                anyhow::bail!("--worktree is not supported with `scodex exec resume`");
            }
            Some(ExecCommand::Review(_)) => {
                anyhow::bail!("--worktree is not supported with `scodex exec review`");
            }
