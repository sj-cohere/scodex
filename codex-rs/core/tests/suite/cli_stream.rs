...
fn personal_access_token_exec_command(server: &MockServer, home: &TempDir) -> Command {
    let bin = codex_utils_cargo_bin::cargo_bin("scodex").unwrap();
...
// Use this for new `scodex exec` subprocess tests in this file. These commands
...
/// Verify that `scodex exec --profile ...` preserves the active user config
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = codex_utils_cargo_bin::cargo_bin("scodex").unwrap();
