...
setup_remote_env() {
  ...
  codex_binary_path="${CARGO_TARGET_DIR:-${REPO_ROOT}/codex-rs/target}/debug/scodex"
  ...
  (
    ...
    cargo build -p codex-cli --bin scodex
  )

  if [[ ! -f "${codex_binary_path}" ]]; then
    echo "scodex binary not found at ${codex_binary_path}" >&2
    return 1
  fi
